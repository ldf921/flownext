...
# def upconvolve(opName, inputLayer, inputChannel, outputChannel, kernelSize, stride, targetH, targetW, stddev=1e-2):
#     with tf.variable_scope(opName, values=[inputLayer]):
#         W = tf.get_variable('W', shape=[kernelSize, kernelSize, outputChannel, inputChannel], dtype=tf.float32, initializer=tflearn.initializations.get('uniform_scaling')() )
#         batchSize = tf.shape(inputLayer)[0]
#         inference = tf.nn.conv2d_transpose(inputLayer, W, tf.concat([tf.stack([batchSize, targetH, targetW]), tf.constant([outputChannel]) ], 0), strides=[1, stride, stride, 1], padding='SAME')
...
class MultiGPUs:
    ...
    def __call__(self, net, args, opt=None):
        ...
        for i in range(self.num):
            def auto_gpu(opr):
                ...
                #     print(opr)
                if opr.type.startswith('Gather') or opr.type in ('L2Loss', 'Pack', 'Gather', 'Tile', 'ReconstructionWrtImageGradient', 'Softmax', 'FloorMod'):
                    return '/cpu:0'
                else:
                    return '/gpu:%d' % i
            with tf.device(auto_gpu):
                ...
                if opt is not None:
                    grads.append(opt.compute_gradients(result['loss'], var_list=net.trainable_variables) )

        with tf.device('/gpu:0'):
            concat_result = {}
            for k in results[0]:
                if len(results[0][k].shape) == 0:
                    concat_result[k] = tf.stack([result[k] for result in results])
                else:
                    concat_result[k] = tf.concat([result[k] for result in results], axis = 0)
    ...
    def call(self, net, kwargs):
        if net.replicated_inputs is None:
            with tf.device('/gpu:0'):
                net.replicated_inputs = dict([(k, self.reshape(v) ) for k, v in net.inputs.items() ] )
    ...
    def average_gradients(grads):
        ...
        for grad_list in zip(*grads):
            ...
            if grad is None:
                ...
                ret.append( (tf.add_n([grad for grad, _ in grad_list]) / len(grad_list), var) )
        return ret


    def reshape(self, tensor):
        shape = tf.shape(tensor)
        list_shape = tensor.shape.as_list()
        single_shape = [ shape[i] if list_shape[i] is None else list_shape[i] for i in range(1, len(tensor.shape)) ]
        return tf.reshape(tensor, tf.stack([self.num, shape[0] // self.num] + single_shape) )
